...
`publish-edge.yml` is manual-dispatch only and uses the protected `edge-production` GitHub environment. It checks out the requested tag, requires a published non-prerelease GitHub release, downloads the exact reviewed release artifacts, and verifies every SHA-256 checksum before using Microsoft's v1.1 Edge Add-ons Update API. It never rebuilds a replacement ZIP for Store deployment. `upload-only` is the safe default. `submit` uploads that same package and submits it for certification using the supplied certification notes.
...
4. Dispatch **Publish Edge update** with the same tag and leave the action at `upload-only`.
