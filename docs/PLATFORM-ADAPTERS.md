...
`transcriptInsights.js` is the first provider-neutral feature module. Teams and Google Meet pass normalized transcript records to the same evidence-summary feature when a meeting ends. The feature assigns stable handoff-only caption IDs, requires claims to cite those IDs, honors managed AI policy, and stages text in the existing local review screen rather than sending transcript content to a provider. `evidenceBoard.js` now consumes the same normalized records for user-created decisions, actions, questions, risks, follow-ups, and important moments. Evidence markers are separate local derivatives and never rewrite the normalized transcript. Future features such as topic chapters and decision ledgers should continue through this boundary instead of reading meeting DOM.
